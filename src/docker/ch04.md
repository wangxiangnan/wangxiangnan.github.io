...
> [!WARNING] 注意
> 生产环境中尽量避免使用 `docker kill`。你的应用应该正确处理 `SIGTERM` 信号，实现优雅退出。如果 `docker stop` 超时后自动 `kill` 了你的容器，说明你的应用没有正确处理退出逻辑。
...
> [!TIP] 提示
> 在生产环境中，务必配置日志轮转（`max-size` 和 `max-file`）。没有日志轮转的容器就像一个永远在写入但从不归档的日志文件——早晚撑爆你的磁盘。
...
> [!TIP] 提示
> `docker cp` 适合临时调试。如果你经常需要在容器内外同步文件，应该使用 Bind Mount（第7章会详细讲）。
...
> [!WARNING] 注意
> 如果你的容器入口命令是 shell 脚本（如 `sh -c "node app.js"`），SIGTERM 会被 shell 接收而不是你的应用！因为 shell 是 PID 1。解决方案：使用 `exec` 形式而不是 shell 形式，或者在 Dockerfile 中使用 `ENTRYPOINT` 而不是 `CMD`（第5章会详细讲）。
